...
string Printer::printResult(vector<string> result) {
...
string Printer::printResult(vector<int> result) {
    string result_string = "[";
    if (!result.empty())
...
template<typename T>
string Printer::printResult(vector<vector<T>> result) {
    string result_string = "[";
    if (!result.empty()) {
        //result_string += "\"" + to_string(result[0]) + "\"";
...
template<typename T>
void Printer::print_element(const T& elem, int indent = 0) {
...
// 递归打印 vector
template<typename T>
void Printer::print_vector(const std::vector<T>& vec, int indent = 0) {
    std::string indentation(indent, ' ');
...
// 专门处理嵌套 vector 的递归打印
template<typename T>
void Printer::print_element(const std::vector<T>& vec, int indent = 0) {
